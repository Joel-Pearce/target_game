...
class Target(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.new_x <= 50:
            self.rightwards = True
            self.leftwards = False
        if self.new_y >= (screen_height - 100):
            self.upwards = True
            self.downwards = False
        if self.new_y <= 50:
            self.downwards = True
...
big_background = pygame.image.load("hoc.png").convert_alpha()
background = pygame.transform.scale(big_background, (screen_width, screen_height))
...
def title_screen():
    ...
    title_surface = title_font.render('Target Quest', False, (255, 255, 255))
    ...
    while running:
        ...
        screen.blit(title_surface, ((screen_width/2) - 430, (screen_height/2) - 200))
